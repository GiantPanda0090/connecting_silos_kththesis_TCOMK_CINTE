#!/usr/bin/python3
#
# ./custom-column_visibility.py course_id column_id true/false
#
#  hide a given custom column (by id) in a given course
#
# G. Q. Maguire Jr.
#
# 2017.01.28
...
def set_custom_columns_hidden(course_id, column_id, column_hidden):
    global Verbose_Flag
    ...
    url = baseUrl + '%s/custom_gradebook_columns/%s' % (course_id, column_id)
    if Verbose_Flag:
       print("url: " + url)
    payload={'column[hidden]': column_hidden}
    r = requests.put(url, headers = header, data=payload)
    write_to_log("result of post updating custom column: " + r.text)
    if r.status_code == requests.codes.ok:
       page_response=r.json()
       print("update column")
       return True
...
def main():
       ...
       if (len(remainder) < 2):
              print("Insuffient arguments\n must provide course_id column_id column_hidden(true/false)\n")
              return

       course_id=remainder[0]
       column_id=remainder[1]
       column_hidden=remainder[2]
       result=set_custom_columns_hidden(course_id, column_id, column_hidden)
