#!/usr/bin/python3
#
# ./list-course-discussion_topics.py course_id [start_date [end-date]]
# with the dates in YYYY-MM-DD format
#
# outputs the course discussion topics for the specified course, optionally limited to the range of dates specified
#
# G. Q. Maguire Jr.
#
# 2017.03.24
...
def list_discussion_topics_date_range(course_id, start_date, end_date):
       topics_found_thus_far=[]

       # Use the Canvas API to get the list of discussion topics for the course
       # GET /api/v1/courses/:course_id/discussion_topics

       url = baseUrl + '%s/discussion_topics' % (course_id) + '?context_codes[]=course_%s&start_date=%s&end_date=%s' % (course_id, start_date, end_date)
       if Verbose_Flag:
              print("url: " + url)

       r = requests.get(url, headers = header)
       if Verbose_Flag:
              write_to_log("result of getting discussion topics: " + r.text)

       if r.status_code == requests.codes.ok:
              page_response=r.json()

       for p_response in page_response:  
              topics_found_thus_far.append(p_response)
...
def list_discussion_topics(course_id):
       ...
       url = baseUrl + '%s/discussion_topics' % (course_id)
       if Verbose_Flag:
              print("url: " + url)
...
def main():
       ...
       if (len(remainder) < 1):
              ...
              if (len(remainder) == 1):
                     ...
                     output=list_discussion_topics(course_id)
              else:
                     course_id=remainder[0]
                     if (len(remainder) == 3):
                     ...
                     output=list_discussion_topics_date_range(course_id, start_date, end_date)
