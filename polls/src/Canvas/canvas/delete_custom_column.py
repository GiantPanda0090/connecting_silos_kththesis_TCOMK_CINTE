#!/usr/bin/python3
#
# ./delete-custom-column.py  course_id column_id
# 
# G. Q. Maguire Jr.
#
# 2016.11.25
...
def delete_custom_column_entries(course_id, column_id):
       ...
       if Verbose_Flag:
              write_to_log("result of deleting custom_gradebook_column: " + r.text)
       page_response=""
...
def list_custom_columns(course_id):
       ...
       while r.links['current']['url'] != r.links['last']['url']:  
              r = requests.get(r.links['next']['url'], headers=header)  
              if Verbose_Flag:
                     write_to_log("getting paginated custom_gradebook_columns: " + r.text)
              page_response = r.json()  
              for p_response in page_response:  
                     columns_found_thus_far.append(p_response)
...
def main(course_id,column_id):
       ...
       if (len(remainder) < 2):
              print("Inusffient arguments\n must provide course_id column_id\n")
              output = delete_custom_column_entries(course_id, column_id)
              if (output):
                     return(output)
       else:
