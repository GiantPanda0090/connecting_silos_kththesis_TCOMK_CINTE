...
def main():
    ...
    if (len(remainder) < 2):
        print("Inusffient arguments\n must provide calendar_event_id participant_id [comment]\n")
        return

    calendar_event_id=remainder[0]
    participant_id=remainder[1]
    if (len(remainder) == 3):
        comment=remainder[2]
        make_appointment_with_comment(calendar_event_id, participant_id, comment)
    else:
        make_appointment_with_comment(calendar_event_id, participant_id,  "appointment made by instructor")
