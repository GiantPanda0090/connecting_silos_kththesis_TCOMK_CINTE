#!/usr/bin/python3
#
# ./list_students_and_observers_in_course.py course_id
#
# outputs a spreadsheet of students and observers in a course as an xlsx file of the form: students-in-189.xlsx
#
# Extensive use is made of Python Pandas merge operations.
# 
# G. Q. Maguire Jr.
#
# based upon earlier list_students_in_course.py
# 2017.08.17
#
...
def observers_in_course(course_id):
       observers_found_thus_far=[]
       ...
       # enrollment_type[] should be set to 'observer'
       # include[] perhaps include email, enrollments, avatar_url
       extra_parameters={'enrollment_type[]': 'observer', 'include[]': 'email, enrollments, avatar_url'}
       r = requests.get(url, params=extra_parameters, headers = header)
       if Verbose_Flag:
              write_to_log("result of getting observer enrollments: " + r.text)

       if r.status_code == requests.codes.ok:
              page_response=r.json()

       for p_response in page_response:  
              observers_found_thus_far.append(p_response)
       ...
       if 'link' in r.headers:
              while r.links['current']['url'] != r.links['last']['url']:  
                     ...
                     for p_response in page_response:  
                            observers_found_thus_far.append(p_response)
       return observers_found_thus_far
...
def main():
       ...
       if (len(remainder) < 1):
              ...
              observers=observers_in_course(course_id)
              observers_df=pd.io.json.json_normalize(observers)

              enrolled_students=enrollments_in_course(course_id)
              enrolled_students_df=pd.io.json.json_normalize(enrolled_students)
              ...
              observers_df.to_excel(writer, sheet_name='Observers')
              enrolled_students_df.to_excel(writer, sheet_name='Enrollments')
