...
def main():
       ...
       students_df = pd.read_excel(open('students-in-'+str(course_id)+'-augmented.xlsx', 'rb'), sheet_name='Students')

       for i in range(len(remainder)-1):
              column_name=remainder[i+1]
              ...
              if Verbose_Flag:
                     print('data type ', students_df.dtypes[column_name])

              if students_df.dtypes[column_name] == 'datetime64[ns]': # if data values are dates, then format as a string
                     for j in range(0,students_df.id.size):
                            user_id=students_df.id.values[j]
                            data_to_store=time.strftime("%Y-%m-%d", time.gmtime(students_df[column_name].values[j].astype(int)/1000000000))
                            put_custom_column_entries(course_id, column_number, user_id, data_to_store)
              elif students_df.dtypes[column_name] == 'object': # if data values are strings these look like "objects"
                     for j in range(0,students_df.id.size):
                            user_id=students_df.id.values[j]
                            data_to_store=students_df[column_name].values[j]
                            put_custom_column_entries(course_id, column_number, user_id, data_to_store)
              elif students_df.dtypes[column_name] == 'int64': # deal with 64 bit integers
                     for j in range(0,students_df.id.size):
                            user_id=students_df.id.values[j]
                            data_to_store=students_df[column_name].values[j]
                            if math.isnan(data_to_store): # do not store NaN values
                                   continue
                            data_to_store=str(int(data_to_store))
                            put_custom_column_entries(course_id, column_number, user_id, data_to_store)
              elif students_df.dtypes[column_name] == 'float64': # deal with floats
                     for j in range(0,students_df.id.size):
                            user_id=students_df.id.values[j]
                            data_to_store=students_df[column_name].values[j]
                            if math.isnan(data_to_store): # do not store NaN values
                                   continue
                            if data_to_store.is_integer(): # if floats are actually integers just at the integer value
                                   data_to_store=str(int(data_to_store))
                            put_custom_column_entries(course_id, column_number, user_id, data_to_store)
              else:
