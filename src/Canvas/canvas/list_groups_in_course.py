...
def main():
       ...
       if (len(remainder) < 1):
              ...
              merged_left_df = pd.merge(left=merge_df,right=students_df, how='left', left_on='member_0', right_on='id')

              old_names = ['id', 'login_id', 'name_y', 'short_name', 'sortable_name'] 
              new_names = ['member_0_id', 'member_0_login_id', 'member_0_name_y', 'member_0_short_name', 'member_0_sortable_name'] 
              merged_left_df.rename(columns=dict(zip(old_names, new_names)), inplace=True)

              merged_left_df2 = pd.merge(left=merged_left_df,right=students_df, how='left', left_on='member_1', right_on='id')

              old_names = ['id', 'login_id', 'name_y', 'short_name', 'sortable_name'] 
              new_names = ['member_1_id', 'member_1_login_id', 'member_1_name_y', 'member_1_short_name', 'member_1_sortable_name'] 
              merged_left_df2.rename(columns=dict(zip(old_names, new_names)), inplace=True)
