#!/usr/bin/python3
#
# ./insert_appointment_by_group.py course_id appointment_group appointment_spreadsheet.csv
#
# Take in a CSV with a list of the format:
# local_start_time,group
# 2017-01-10 14:30,Project Groups P1P2 3
#  ...
#
# 
# We assume that the groups are from the set of "Project Groups".
#
...
def main():
    ...
    appointments_to_make_df.rename(columns = {'utc_local_start_time':'start_at', 'group': 'group_name'}, inplace = True)
    ...
    groups_df.rename(columns = {'name':'group_name', 'id': 'group_id'}, inplace = True)

    merge_df = pd.merge(appointments_to_make_df, groups_df, on='group_name')

    group_membership=[]
    for g in groups:
        for m in members_of_groups(g['id']):
            gmem={'member': m, 'group_id': g['id'], 'name': g['name']}
            group_membership.append(gmem)

    membership_df=pd.io.json.json_normalize(group_membership)

    merge2_df = pd.merge(merge_df, membership_df, on='group_id')
    ...
    merged_new_appointments_df=pd.merge(merge2_df, ag_df, on='start_at')
    merged_new_appointments_df.rename(columns = {'member':'user_id', 'id': 'calendar_event_id'}, inplace = True)
    ...
    merge2_df.to_excel(writer, sheet_name='MergedGroupsApp')
