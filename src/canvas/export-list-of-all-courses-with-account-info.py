#!/usr/bin/python3
#
# ./export-list-of-all-courses-with-account-info.py account_id
#
# outputs the list of courses for the specified account course together with account information
#
# G. Q. Maguire Jr.
#
# 2017.04.01
...
def main():
       ...
       # get the account information, so that one can later augment the course information with this account information
       ...
       if (output):
              ...
              account_df=pd.io.json.json_normalize(output)

              nice_account_names=dict()
              account_df['nice_account_name']=' '
              for index, row in  account_df.iterrows():
                     if Verbose_Flag:
                            print('index: ', index, 'row[id]: ', row['id'])
                     if row['parent_account_name'] is None:
                            nice_account_name=row['name']
                     else:
                            nice_account_name=row['parent_account_name']+'_'+row['name']
                     nice_account_names[row['id']]=nice_account_name
                     account_df.set_value(index, 'nice_account_name', nice_account_name)

              if Verbose_Flag:
                     print('nice_account_names=', nice_account_names)
      
       output=list_courses_in_account(account_id)
       if (output):
              if Verbose_Flag:
                     print('length of output: ', len(output))
                     pprint(output)
       ...
       writer = pd.ExcelWriter('courses-'+str(account_id)+'augmented.xlsx', engine='xlsxwriter')

       if account_df is not None:
              account_df.to_excel(writer, sheet_name='Accounts')

       if (account_df is not None) and (df is not None):
              df['nice_account_name']=' '
              for index, row in  df.iterrows():
                     if Verbose_Flag:
                            print('index: ', index, 'row[account_id]: ', row['account_id'])
                     nice_account_name=nice_account_names[row['account_id']]
                     df.set_value(index, 'nice_account_name', nice_account_name)

       if df is not None:
              # Convert the dataframe to an XlsxWriter Excel object.
              df.to_excel(writer, sheet_name='Courses')


       #       merge_df = pd.merge(account_df, df, on='account_id')
       #       merge_df.to_excel(writer, sheet_name='CoursesAug')
